...
Circle.colortable = {
	[Moan.osc.rect]     = {200,  0,  0,200},
	[Moan.osc.saw]      = {200,150,  0,200},
	[Moan.osc.triangle] = {  0,200,  0,200},
	[Moan.osc.sin]      = {  0,  0,200,200},
}
Circle.nextosc = {
	[Moan.osc.rect]     = Moan.osc.sin,
	[Moan.osc.saw]      = Moan.osc.rect,
	[Moan.osc.triangle] = Moan.osc.saw,
	[Moan.osc.sin]      = Moan.osc.triangle,
}
function Circle.new(c)
	local osc = c.osc or Moan.osc.sin
	local t = {
...
function Circle:newsample()
	...
	self.sample = Moan.newSample(Moan.compress(Moan.envelope(
		self.osc(self.pos.x + 20, amp),
		Moan.osc.triangle(8),
		Moan.env.adsr(len/10, 3*len/10, 5*len/10, len/10, amp*1.1, amp))), len)
end
